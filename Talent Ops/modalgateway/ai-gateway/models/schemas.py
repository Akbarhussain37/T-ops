...
class ChatResponse(BaseModel):
    ...
    @validator('answer')
    def validate_grounding(cls, v, values):
        """Ensure answers are grounded in sources"""
        if not values.get('out_of_scope') and not values.get('source_chunks'):
            raise ValueError("Non out-of-scope answers must have source_chunks")
        return v
